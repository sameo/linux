			     ...
			     struct device_node *parent)
{
	...
	rc = of_property_read_u32(node, "riscv,guest-index-bits",
	...
	rc = of_property_read_u32(node, "riscv,hart-index-bits",
	...
	rc = of_property_read_u32(node, "riscv,group-index-bits",
	...
	rc = of_property_read_u32(node, "riscv,group-index-shift",
	...
	rc = of_property_read_u32(node, "riscv,num-ids", &global->nr_ids);
	if (rc) {
	...
	if (of_property_read_u32(node, "riscv,num-guest-ids",
	    &global->nr_guest_ids))
		global->nr_guest_ids = global->nr_ids;
	if ((global->nr_guest_ids < IMSIC_MIN_ID) ||
	    (global->nr_guest_ids >= IMSIC_MAX_ID) ||
	    ((global->nr_guest_ids & IMSIC_MIN_ID) != IMSIC_MIN_ID)) {
		pr_err("%pOFP: invalid number of guest interrupt identities\n",
		       node);
		return -EINVAL;
	}

	/* Check if IPIs are slow */
	priv->slow_ipi = of_property_read_bool(node, "riscv,slow-ipi");
	...
	domain = irq_find_matching_fwnode(riscv_get_intc_hwnode(),
					  DOMAIN_BUS_ANY);
	if (!domain) {
		pr_err("%pOFP: Failed to find INTC domain\n", node);
		rc = -ENOENT;
		goto out_ids_cleanup;
	}
	imsic_parent_irq = irq_create_mapping(domain, RV_IRQ_EXT);
	if (!imsic_parent_irq) {
		pr_err("%pOFP: Failed to create INTC mapping\n", node);
		rc = -ENOENT;
		goto out_ids_cleanup;
	}
	...
	rc = imsic_irq_domains_init(priv, &node->fwnode);
	if (rc) {
	...
	pr_info("%pOFP:  hart-index-bits: %d,  guest-index-bits: %d\n",
		node, global->hart_index_bits, global->guest_index_bits);
	pr_info("%pOFP: group-index-bits: %d, group-index-shift: %d\n",
		node, global->group_index_bits, global->group_index_shift);
	pr_info("%pOFP: mapped %d interrupts for %d CPUs at %pa\n",
		node, global->nr_ids, nr_handlers, &global->base_addr);
	if (priv->ipi_lsync_id)
		pr_info("%pOFP: enable/disable sync using interrupt %d\n",
			node, priv->ipi_lsync_id);
	if (priv->ipi_id)
		pr_info("%pOFP: providing IPIs using interrupt %d\n",
			node, priv->ipi_id);
